...
<head>
	...
	<style>
		...
		.trainer-controls { display: grid; gap: 12px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
		.trainer-controls label { align-items: flex-start; display: grid; gap: 6px; }
		.segmented { display: flex; flex-wrap: wrap; gap: 8px; }
		.segmented label { background: #0e1013; border: 1px solid var(--line); border-radius: 4px; cursor: pointer; display: inline-flex; padding: 8px 10px; }
		.segmented input { margin: 0; }
		.phrase-prompt { font-size: 1.8rem; line-height: 1.25; margin: 14px 0 10px; overflow-wrap: anywhere; }
		.phrase-hint { color: #f1d18f; font-size: 1.05rem; min-height: 1.6rem; }
		.phrase-hint.hidden { visibility: hidden; }
		.phrase-answer { box-sizing: border-box; font-size: 1.2rem; width: 100%; }
		.phrase-answer.wrong { border-color: #d08a8a; }
		.phrase-progress { color: var(--muted); display: flex; flex-wrap: wrap; gap: 10px; justify-content: space-between; }
		.phrase-done { border-left: 4px solid var(--focus); padding: 8px 12px; background: #151b24; }
		@media (max-width: 700px) { .trainer-controls { grid-template-columns: 1fr; } }
...
<div class="panel">
	<h2>Trainers</h2>
	<p><a href="/phrasing">Phrasing trainer</a></p>
</div>
<h2>Import</h2>
...
<section class="panel">
	<div class="trainer-controls">
		<label>Progression
			<select id="phrase-lesson"></select>
		</label>
		<label>Verb focus
			<select id="phrase-verb"></select>
		</label>
		<label>Random session length
			<input type="number" id="phrase-count" min="1" max="500" value="40">
		</label>
		<label>Hints
			<select id="phrase-hints">
				<option value="after" selected>after mistake</option>
				<option value="on">always on</option>
				<option value="off">off</option>
			</select>
		</label>
	</div>
	<div class="actions segmented" role="radiogroup" aria-label="Practice mode">
		<label><input type="radio" name="phrase-mode" value="ordered" checked> Current lesson</label>
		<label><input type="radio" name="phrase-mode" value="cumulative"> Cumulative random</label>
	</div>
	<div class="actions">
		<button type="button" id="phrase-restart" class="primary">Restart</button>
		<button type="button" id="phrase-reroll">Shuffle</button>
		<span class="small" id="phrase-set-summary"></span>
	</div>
</section>
<section class="panel" id="phrase-trainer">
	<div class="phrase-progress">
		<span id="phrase-progress"></span>
		<span id="phrase-lesson-name"></span>
	</div>
	<div class="phrase-prompt" id="phrase-prompt"></div>
	<div class="phrase-hint hidden" id="phrase-hint"></div>
	<input class="phrase-answer" id="phrase-answer" spellcheck="false" autocomplete="off" autocapitalize="off" autocorrect="off">
	<p class="small" id="phrase-status" aria-live="polite"></p>
</section>
<script>
