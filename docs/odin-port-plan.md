...
disabled with `--no-trace-strokes`, trace key events are available with
`--trace-key-events`, and translation timing reports first-output latency with
`--time-translations`, matching the C CLI. Raw serial diagnostics are wired
